...
namespace ScriptEd
{
    public partial class addString : Form
    {
        ...
        private void getPrev()
        {
            ...
            prevText = prevText.Remove(prevText.Length - 4);
        ...
        private void getBox()
        {
            ...
            int lines = stResult.Text.Split('\n').Length;
            int i = 0, j = 0, start = 0, end = 0;
            bool endBox = false, readComm = false, Comm0 = false;
            char read;
            int[] colorpos = new int[lines];
            int[] colorval = new int[lines];
            List<Color> colorList = new List<Color>();
            List<int> colorChar = new List<int>();
            while (endBox == false)
            {
                read = prevText[i];
                switch (read)
                ...
                if (readComm == false && Comm0 == true)
                {
                    switch (comm)
                    {
                        ...
                        default:       colorList.Add(setColor.TextToColor(comm));
                                       colorChar.Add(start);
                                       break;
                    }
                    comm = "";
            ...
            prev.Text = show;
            int iter = 0;
            while (iter < colorList.Count)
            {
                prev.Select(colorChar[iter], prev.TextLength - colorChar[iter]);
                prev.SelectionColor = colorList[iter];
                iter++;
            }
        ...
        private void initString(object sender, EventArgs e)
        {
            ...
            strId.Text = Program.stringId;
            stringBox.Text = Program.sendString;
        }
        ...
        private void colorText(Color color1, string color2Name)
        {
            ...
            int startSel = stringBox.SelectionStart;
            stringBox.Text = stringBox.Text.Insert(startSel, "[" + color2Name + "]");
            /*stringBox.Select(startSel, stringBox.TextLength - startSel);
            stringBox.SelectionColor = color2;*/
            stringBox.Select(startSel, 0);
        }
